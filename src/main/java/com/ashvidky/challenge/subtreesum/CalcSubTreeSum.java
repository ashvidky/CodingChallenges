package com.ashvidky.challenge.subtreesum;

import java.util.ArrayList;

public class CalcSubTreeSum {
...
//		System.out.println(getMinSubtreeSumDifference(vertex, edges, N));
		...
		System.out.println(calc(vertex, edges, N));
	}

	public static int calc(int vertex[], int[][] edges, int N) {
		...
		int sum  = calcTotal(edge, vertex, 0, -1);
		return sum;
	}

	private static int calcTotal(ArrayList<Integer>[] edge, int[] vertex, int node, int parent) {
		
		int sum = vertex[node];
		
		ArrayList<Integer> children = edge[node];
		for (Integer child : children) {
			if (child != parent)
				sum += calcTotal(edge, vertex, child, node);
		}
